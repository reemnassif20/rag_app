...
# Streamlit UI
st.title("📄 RAG Application with Langchain, FAISS & Ollama")
...
# Load existing vector store if no files are uploaded but store exists
if not vectorstore: # if vectorstore wasn't created/updated by new uploads
...
if st.sidebar.button("Clear Uploaded Files and Vector Store"):
    # Clear uploaded files
    if os.path.exists(config.UPLOAD_DIR):
        for f_name in os.listdir(config.UPLOAD_DIR):
            file_to_remove = os.path.join(config.UPLOAD_DIR, f_name)
            try:
                os.remove(file_to_remove)
            except Exception as e:
                st.sidebar.error(f"Error removing {file_to_remove}: {e}")
        st.sidebar.success("Uploaded files directory cleared (or attempted).")
    ...
    if os.path.exists(config.VECTORSTORE_PATH):
        try:
            ...
            else: # if it's a file (older FAISS versions might save as single file)
